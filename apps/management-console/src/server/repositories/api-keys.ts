...
export const createApiKeysRepository = (db: ManagementConsoleDatabase) => ({
  ...
  findByKeyPrefix: async (keyPrefix: string) => {
    const [record] = await db.select().from(apiKeys).where(eq(apiKeys.keyPrefix, keyPrefix)).limit(1);

    return record ?? null;
  },
  list: async () => db.select().from(apiKeys),
  markUsed: async (id: string, lastUsedAt = new Date()) => {
    const [record] = await db.update(apiKeys).set({ lastUsedAt }).where(eq(apiKeys.id, id)).returning();
