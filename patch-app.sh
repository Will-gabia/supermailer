cat << 'PATCH' > app.patch
--- apps/management-console/src/client/App.tsx
+++ apps/management-console/src/client/App.tsx
@@ -74,6 +74,27 @@
   createdAt: string;
 };
...
@@ -1035,11 +1095,96 @@
           ...
           <ul data-testid="send-list">
             {sends.map((send) => (
-              <li key={send.id} data-testid={`send-${send.id}`}>
-                {send.kind} — {send.recipientEmail} — {send.status} — {send.queueJobId ?? 'not_queued'}
...
+          )}
         ...
         <section>
PATCH
patch apps/management-console/src/client/App.tsx < app.patch
